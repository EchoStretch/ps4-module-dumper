...
int dump_dir_macro(const char *src, const char *dest, const char *root) {
    char output_loc[PATH_MAX] = {0};
    printf_notification("Dumping %s", src);
    snprintf_s(output_loc, sizeof(output_loc), dest, root);
    decrypt_dir((char *)src, output_loc);  // Cast away const

    return 0;
}

int dump_file_macro(const char *src, const char *dest, const char *root) {
    char output_loc[PATH_MAX] = {0};
    printf_notification("Dumping %s", src);
    snprintf_s(output_loc, sizeof(output_loc), dest, root);
    decrypt_and_dump_self((char *)src, output_loc);  // Cast away const
...
int _main(struct thread *td) {
    ...
    char temp_output_root[PATH_MAX] = {0};  // Temporary buffer
    ...
    snprintf_s(temp_output_root, sizeof(temp_output_root), "%s/%s", output_root, fw_version);
    snprintf_s(output_root, sizeof(output_root), "%s", temp_output_root);  // Use temp buffer to avoid aliasing
    mkdir(output_root, 0777);
    snprintf_s(temp_output_root, sizeof(temp_output_root), "%s/modules", output_root);
    snprintf_s(output_root, sizeof(output_root), "%s", temp_output_root);  // Use temp buffer to avoid aliasing
